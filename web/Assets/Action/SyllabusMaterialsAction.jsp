<%-- 
    Document   : syllabusMaterials
    Created on : Aug 17, 2022, 6:17:19 PM
    Author     : Angel
--%>
...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("fileimage"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\SyllabusMaterials\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            photo = "syllabus_pdf_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                     ...
                     if (field_name.equals("fileimage2"))//write the name of image here it is img_product
                        {
                            String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            ...
                            proof = "syllabus_pdf_Tumb" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
...
//                        }
           ...
           //to upload proof of doctor
                ...
                String InsQry = "insert into tbl_syllabus(internship_id,syllabus_material,syllabus_tumbnail)values('"+value[0]+"','"+photo+"','"+proof+"')";
  ...
  // System.out.println(InsQry);
       ...
       <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Company/AddSyllabus.jsp';

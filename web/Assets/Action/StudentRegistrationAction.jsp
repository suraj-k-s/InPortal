<%-- 
    Document   : StudentRegistrationAction
    Created on : Aug 15, 2022, 10:12:55 PM
    Author     : Angel
--%>
...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\StudentPhoto\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            photo = "student_photo_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("proof"))//write the name of image here it is img_product
                        {
                            ...
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\StudentProof\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            proof = "student_proof_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
...
//                        }
           ...
           //to upload proof of doctor
                ...
                String InsQry = "insert into tbl_student(student_name,student_email,student_contact,student_gender,student_address,student_password,place_id,student_location,student_photo,student_proof,student_doj)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[4]+"','"+value[5]+"','"+value[9]+"','"+value[10]+"','"+photo+"','"+proof+"','"+value[11]+"')";
  ...
  // System.out.println(InsQry);
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Guest/StudentRegistration.jsp';

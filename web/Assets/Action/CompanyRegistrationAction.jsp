<%-- 
    Document   : CompanyRegistrationAction
    Created on : Aug 15, 2022, 3:54:13 PM
    Author     : Angel
--%>
...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("logo"))//write the name of image here it is img_product
                        {
                            ...
                            photo = "company_logo_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("proof"))//write the name of image here it is img_product
                        {
                            ...
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\CompanyProof\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            proof = "company_proof_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
...
//                        }
           ...
           //to upload proof of doctor
                ...
                String InsQry = "insert into tbl_company(company_name,company_contact,company_email,company_address,company_password,place_id,company_location,company_landmark1,company_landmark2,company_logo,company_proof,company_liscenceno,company_doj,sector_id,company_site,company_about,company_date)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[4]+"','"+value[11]+"','"+value[12]+"','"+value[13]+"','"+value[14]+"','"+photo+"','"+proof+"','"+value[15]+"',curdate(),'"+value[6]+"','"+value[7]+"','"+value[8]+"','"+value[16]+"')";

...
<html>
    <head>
        ...
        <title>InPortal::Company Sector</title>
  ...
  <%
         ...
         if (request.getParameter("btnsubmit") != null) {
              if(!request.getParameter("txtid").equals(""))
                {
                String sedit = "update tbl_sector set sector_name='"+request.getParameter("txt_sector")+"' where sector_id='"+request.getParameter("txtid")+"'";
                
                con.executeCommand(sedit);
                response.sendRedirect("CompanySector.jsp");
                
                }
             else{   
         String query = "insert into tbl_sector(sector_name) values('" + request.getParameter("txt_sector") + "')";
          if (con.executeCommand(query)) {
        %>
        <script>
            alert("Data Inserted");
            window.location = "CompanySector.jsp";
        </script>
        <%
        } else {
        %>
       ...
       String dl = "delete from tbl_sector where sector_id='"+request.getParameter("did")+"'";
        ...
        <!--delete code ends-->
         ...
         String sedit="select * from tbl_sector  where sector_id='"+request.getParameter("eid")+"'";
        
        ResultSet rs2 = con.selectCommand(sedit);
       
        rs2.next();
        sid=rs2.getString("sector_id");
        sname=rs2.getString("sector_name");
...
<div class="inner-block" >
    <div style="display: inline-flex;">
      ...
      <fieldset>
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control" name="txt_sector" value="<%=sname%>" autocomplete="off" id="txt2" required>
        </div>
          <br>
  ...
  <table class="table table-striped table-bordered" style="width: 43em;" id="example">
           ...
           String sview="select * from tbl_sector";
            ...
            <tbody >
                ...
                <tr id="blink">
                    ...
                    <td>
                        <a href="CompanySector.jsp?did=<%=rs1.getString("sector_id")%>"><button type="button" class="btn btn-danger">Delete</button></a> <a href="CompanySector.jsp?eid=<%=rs1.getString("sector_id")%>"><button type="button" class="btn btn-primary">Edit</button></a>  
                    </td>
                </tr>
                <%
                }
